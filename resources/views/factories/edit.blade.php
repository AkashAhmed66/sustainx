                ...
                <form action="{{ route('factories.update', $factory) }}" method="POST">
                        ...
                        <div>
                            ...
                            <select name="factory_type_id"
                                ...
                                @foreach($factoryTypes as $type)
                                    <option value="{{ $type->id }}" {{ old('factory_type_id', $factory->factory_type_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="country_id"
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" {{ old('country_id', $factory->country_id) == $country->id ? 'selected' : '' }}>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea
                               ...
                               placeholder="Enter factory address">{{ old('address', $factory->address) }}</textarea>
                        @error('address')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            @forelse($users as $user)
                                <label class="flex items-center py-2 hover:bg-neutral-50 px-2 rounded cursor-pointer">
                                    <input type="checkbox"
                                           name="user_ids[]"
                                           value="{{ $user->id }}"
                                           {{ in_array($user->id, old('user_ids', $factory->users->pluck('id')->toArray())) ? 'checked' : '' }}
                                           class="w-4 h-4 text-primary-600 border-neutral-300 rounded focus:ring-primary-500">
