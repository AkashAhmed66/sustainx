                ...
                <nav class="sidebar-nav overflow-y-auto scrollbar-thin" style="max-height: calc(100vh - 64px);">
                    ...
                    @foreach($sidebarItems as $index => $item)
                        ...
                        @elseif($item['type'] === 'link')
                            ...
                            @if($item['route'] === 'assessments.index' && auth()->user()->can('preview assessments'))
                                <form method="POST" action="{{ route('assessments.preview-first') }}">
                                    @csrf
                                    <button type="submit" class="sidebar-link w-full text-left">
                                        <svg class="sidebar-link-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                        </svg>
                                        <span class="sidebar-link-text">Submit Form</span>
                                    </button>
                                </form>
                            @endif
                        @endif
