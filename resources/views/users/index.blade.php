    ...
    <div class="p-4 sm:p-6">
        <x-data-table
            ...
            <tr class="hover:bg-neutral-50 transition-colors">
                @if($bulkEnabled)
                <td class="px-4 sm:px-6 py-4 w-12">
                    <input type="checkbox"
                ...
                @foreach($columns as $key => $label)
                    ...
                    @elseif($key === 'email')
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-neutral-600">
                            {{ $item->email }}
                        </td>
                    @elseif($key === 'roles')
                        ...
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-2 flex-wrap">
                                <a href="{{ route($config['editRoute'], $item) }}"
                                   class="inline-flex items-center px-2 sm:px-3 py-1.5 bg-primary-50 text-primary-700 rounded-lg hover:bg-primary-100 transition-colors text-xs sm:text-sm">
                                ...
                                <form action="{{ route('users.destroy', $item) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="inline-flex items-center px-2 sm:px-3 py-1.5 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition-colors text-xs sm:text-sm">
                                        <svg class="w-4 h-4 sm:mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @empty
            <tr>
                <td colspan="{{ $bulkEnabled ? count($columns) + 1 : count($columns) }}" class="px-4 sm:px-6 py-12 text-center">
