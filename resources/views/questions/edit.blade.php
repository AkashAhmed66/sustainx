        ...
        <div class="flex flex-col lg:flex-row gap-6"
             x-data="{
                 ...
                 removeConnectionQuestion(optionIndex, connectionIndex) {
                     ...
                     if (bucket.length > 0) bucket.splice(connectionIndex, 1);
                 },
                ...
                openAddConnectionModal(optionIndex) { this.modalOptionIndex = optionIndex; this.modalConnectionIndex = null; this.modalConnection = this.getEmptyConnection(); this.modalOpen = true; },
                openEditConnectionModal(optionIndex, connectionIndex) { this.modalOptionIndex = optionIndex; this.modalConnectionIndex = connectionIndex; const conn = (this.connectionQuestionsByOption[optionIndex] || [])[connectionIndex] || this.getEmptyConnection(); this.modalConnection = JSON.parse(JSON.stringify(conn)); this.modalOpen = true; },
                ...
                saveModalConnection() { if (this.modalConnection.question_text.trim() === '') { alert('Question text is required'); return; } const optIdx = this.modalOptionIndex; if (!this.connectionQuestionsByOption[optIdx]) this.connectionQuestionsByOption[optIdx] = []; if (this.modalConnectionIndex === null) { this.connectionQuestionsByOption[optIdx].push(JSON.parse(JSON.stringify(this.modalConnection))); } else { this.connectionQuestionsByOption[optIdx].splice(this.modalConnectionIndex, 1, JSON.parse(JSON.stringify(this.modalConnection))); } this.closeModal(); },
                modalOpen: false,
                modalOptionIndex: null,
                modalConnectionIndex: null,
                modalConnection: { id: null, question_type_id: '', sl_no: '', question_text: '', input_unit: '', output_unit: '', options: [], child_questions: [], equation_name: '', factors: [], is_required: false, is_active: true },
                 addFactor() {
                     this.factors.push({ sn: this.factors.length + 1, operation: 'multiply', factor_value: '', country_id: '' });
                 },
                 removeFactor(index) {
                     if (this.factors.length > 1) { this.factors.splice(index, 1); this.factors.forEach((fac, idx) => fac.sn = idx + 1); }
                 },
                 addChildQuestion() {
                     this.childQuestions.push({ id: null, question_text: '', input_unit: '', equation_name: '', factors: [{ sn:1, operation:'multiply', factor_value:'', country_id:'' }] });
                 },
                 removeChildQuestion(index) { if (this.childQuestions.length > 1) this.childQuestions.splice(index, 1); },
                 addChildFactor(childIndex) { if (!this.childQuestions[childIndex].factors) this.childQuestions[childIndex].factors = []; this.childQuestions[childIndex].factors.push({ sn: this.childQuestions[childIndex].factors.length + 1, operation: 'multiply', factor_value: '', country_id: '' }); },
                 removeChildFactor(childIndex, factorIndex) { const factors = this.childQuestions[childIndex].factors || []; if (factors.length > 1) { factors.splice(factorIndex, 1); factors.forEach((factor, idx) => factor.sn = idx + 1); } }
             }"
             x-init="syncConnectionBuckets();">
                    ...
                    <div class="grid grid-cols-1 gap-6 mb-6">
                                ...
                                <div>
                                    ...
                                    <select name="depends_on_question_id"
                                        ...
                                        <template x-for="triggerQuestion in filteredTriggerQuestions" :key="triggerQuestion.id">
                                            <option :value="triggerQuestion.id"
                                                    :selected="String(triggerQuestion.id) === String(triggerQuestionId)"
                                                    x-text="triggerQuestion.label"></option>
                                        </template>
                                    </select>
                                    ...
                                    <select name="depends_on_option_id"
                                        ...
                                        <template x-for="triggerOption in availableTriggerOptions" :key="triggerOption.id">
                                            <option :value="triggerOption.id"
                                                    :selected="String(triggerOption.id) === String(triggerOptionId)"
                                                    x-text="triggerOption.option_text"></option>
                                        </template>
                                    </select>
                        ...
                        <div x-show="questionType != '4'">
                            ...
                            <input type="text"
                                   name="input_unit"
                                   id="input_unit"
                                   value="{{ old('input_unit', $question->input_unit) }}"
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="output_unit"
                                   id="output_unit"
                                   value="{{ old('output_unit', $question->output_unit) }}"
                                        ...
                                        <template x-for="(connection, connectionIndex) in (connectionQuestionsByOption[optionIndex] || [])" :key="connectionIndex">
                                            <div class="rounded-xl border border-neutral-200 bg-white p-4">
                                                <div class="flex items-center justify-between gap-3 mb-3">
                                                    <p class="text-sm font-semibold text-neutral-700" x-text="`Connected Question ${connectionIndex + 1}`"></p>
                                                    <button type="button"
                                                            @click="removeConnectionQuestion(optionIndex, connectionIndex)"
                                                            ...
                                                            x-show="(connectionQuestionsByOption[optionIndex] || []).length > 0">
                                                        Remove
                                                    </button>
                                                </div>

                                                <input type="hidden"
                                                       :name="`connection_questions[${optionIndex}][${connectionIndex}][id]`"
                                                       x-model="connection.id">
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <input type="number"
                                                               min="1"
                                                               :name="`connection_questions[${optionIndex}][${connectionIndex}][sl_no]`"
                                                               x-model="connection.sl_no"
                                                               class="w-full rounded-lg border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                                               placeholder="Global serial number">
                                                    </div>

                                                    <div>
                                                        <label class="block text-sm font-medium text-neutral-700 mb-2">Question Text <span class="text-red-500">*</span></label>
                                                        <input type="text"
                                                               :name="`connection_questions[${optionIndex}][${connectionIndex}][question_text]`"
                                                               x-model="connection.question_text"
                                                               class="w-full rounded-lg border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                                               placeholder="Enter connected question text">
                                                    </div>

                                                    <div>
                                                        <label class="block text-sm font-medium text-neutral-700 mb-2">Question Type</label>
                                                        <select :name="`connection_questions[${optionIndex}][${connectionIndex}][question_type_id]`"
                                                                x-model="connection.question_type_id"
                                                                class="w-full rounded-lg border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500">
                                                            <option value="">Select Type</option>
                                                            @foreach($questionTypes as $type)
                                                    ...
                                                    <div>
                                                        <label class="block text-sm font-medium text-neutral-700 mb-2">Input Unit</label>
                                                        <input type="text"
                                                               :name="`connection_questions[${optionIndex}][${connectionIndex}][input_unit]`"
                                                               x-model="connection.input_unit"
                                                               class="w-full rounded-lg border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                                               placeholder="Optional">
                                                    </div>

                                                    <div>
                                                        <label class="block text-sm font-medium text-neutral-700 mb-2">Output Unit</label>
                                                        <input type="text"
                                                               :name="`connection_questions[${optionIndex}][${connectionIndex}][output_unit]`"
                                                               x-model="connection.output_unit"
                                                               class="w-full rounded-lg border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                                               placeholder="Optional">
                                                    </div>
                                                        <div class="md:col-span-2">
                                                            <label class="block text-sm font-medium text-neutral-700 mb-2">Equation Name</label>
                                                            <input type="text"
                                                                   :name="`connection_questions[${optionIndex}][${connectionIndex}][equation_name]`"
                                                                   x-model="connection.equation_name"
                                                                   class="w-full rounded-lg border border-neutral-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
                                                                   placeholder="Optional">
                                                        </div>

                                                        <div class="md:col-span-2" x-show="connection.question_type_id == TYPE_MCQ || connection.question_type_id == TYPE_MULTIPLE_SELECT">
                                                            <label class="block text-sm font-medium text-neutral-700 mb-2">Options</label>
                                                            <template x-for="(opt, optIndex) in (connection.options || [])" :key="optIndex">
                                                                <div class="grid grid-cols-12 gap-3 mb-2 items-start">
                                                                    <div class="col-span-7">
                                                                        <input type="text"
                                                                               :name="`connection_questions[${optionIndex}][${connectionIndex}][options][${optIndex}][option_text]`"
                                                                    ...
                                                                    <div class="col-span-3">
                                                                        <input type="number"
                                                                               step="any"
                                                                               :name="`connection_questions[${optionIndex}][${connectionIndex}][options][${optIndex}][option_value]`"
                                                                               x-model="opt.option_value"
                                                                               class="w-full px-3 py-2 border border-neutral-300 rounded-lg"
                                                                               placeholder="Value (Optional)">
                                                                    </div>
                                                                    <div class="col-span-2">
                                                                        <button type="button" @click="removeConnectionOption(optionIndex, connectionIndex, optIndex)" class="px-2 py-2 text-red-600 hover:bg-red-50 rounded-lg">Remove</button>
                                                                    </div>
                                                                </div>
                                                            </template>
                                                            <button type="button" @click="addConnectionOption(optionIndex, connectionIndex)" class="mt-2 px-3 py-1.5 text-primary-600 bg-primary-50 hover:bg-primary-100 rounded-lg">+ Add Option</button>
                                                        </div>

                                                        <div class="md:col-span-2" x-show="connection.question_type_id == TYPE_MULTIPLE_NUMERIC">
                                                            <label class="block text-sm font-medium text-neutral-700 mb-2">Child Questions (Numeric)</label>
                                                            <template x-for="(child, cIdx) in (connection.child_questions || [])" :key="cIdx">
                                                                <div class="border border-neutral-200 rounded-lg p-3 mb-2 bg-white">
                                                                    <div class="space-y-3">
