                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="factory_id"
                                ...
                                @foreach($factories as $factory)
                                    <option value="{{ $factory->id }}" {{ old('factory_id', $assessment->factory_id) == $factory->id ? 'selected' : '' }}>
                                        {{ $factory->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="period"
                                ...
                                <option value="annual" {{ old('period', $assessment->period) == 'annual' ? 'selected' : '' }}>Annual</option>
                                <option value="quarterly" {{ old('period', $assessment->period) == 'quarterly' ? 'selected' : '' }}>Quarterly</option>
                            </select>
                            @error('period')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="status"
                                ...
                                <option value="submitted" {{ old('status', $assessment->status) == 'submitted' ? 'selected' : '' }}>Submitted</option>
                                <option value="in_review" {{ old('status', $assessment->status) == 'in_review' ? 'selected' : '' }}>In Review</option>
                                <option value="approved" {{ old('status', $assessment->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                                <option value="rejected" {{ old('status', $assessment->status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                            </select>
                            @error('status')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
